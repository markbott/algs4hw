...
public class SAP {
	...
	AncestorInfo findShortestAncestor(int v, int w) {
		checkVertex(v);
		checkVertex(w);
	...
	AncestorInfo findShortestAncestor(BFSResult bfsV, int v, BFSResult bfsW, int w) {	
	...
	private AncestorInfo findShortestAncestor(Iterable<Integer> v, Iterable<Integer> w) {
		...
		for(int a : v) { 
			for(int b : w) {
				...
				if(ancestor.ancestor < 0) continue; // no ancestor here!
